...
export const TopHUD: React.FC<TopHUDProps> = ({
  ...
  return (
    <header
      className="absolute top-2 sm:top-3 left-2 sm:left-3 right-2 sm:right-3 flex items-center justify-between pointer-events-none z-40 gap-1.5"
      onTouchStart={(e) => e.stopPropagation()}
    >
      {/* 左上: タイトル & モード切替 & Undo/Redo & F3 */}
      <div className="flex items-center gap-1.5 sm:gap-2 pointer-events-auto">
        <div className="glass-panel px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-2xl flex items-center gap-2 sm:gap-3 bg-slate-950/80 backdrop-blur-md border border-white/15 shadow-xl">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-cyan-400 animate-pulse"></span>
            <h1 className="font-bold tracking-wider text-sm sm:text-base bg-gradient-to-r from-cyan-300 via-sky-200 to-amber-200 bg-clip-text text-transparent">
              Airas
            </h1>
            <span className="text-[9px] sm:text-[10px] uppercase font-mono px-1 sm:px-1.5 py-0.5 rounded bg-cyan-950/80 text-cyan-300 border border-cyan-500/30">
              HD-2D
            </span>
          </div>
          ...
          <div className="flex items-center p-0.5 bg-black/40 rounded-xl border border-white/5">
            <button
              ...
              className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-2.5 py-1 rounded-lg text-xs font-medium transition-all active:scale-95 cursor-pointer ${
                activeMode === 'play'
          ...
          <button
            ...
            className={`px-2 py-1 rounded-lg border text-[11px] font-mono flex items-center gap-1 transition-all active:scale-95 cursor-pointer ${
              isF3Open
      ...
      {/* 中央: AI World Brain ボタン (スマホではアイコンボタン、大画面ではフルラベル) */}
      <div className="pointer-events-auto">
        <button
          ...
          className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-2xl flex items-center gap-1.5 sm:gap-2 font-medium text-xs sm:text-sm transition-all shadow-lg active:scale-95 cursor-pointer ${
            isAIPanelOpen
        ...
        >
          ...
          <span className="hidden sm:inline">AI世界生成 & 画像創出</span>
          <span className="sm:hidden font-bold text-xs">AI創出</span>
          <span className="text-[10px] bg-cyan-900/80 px-1.5 py-0.5 rounded text-cyan-300 font-mono hidden md:inline">
            ⌘K
          </span>
        </button>
      </div>
      ...
      <div
        ...
        onTouchStart={(e) => e.stopPropagation()}
      >
        {/* 天候セレクター (PCのみフル表示、モバイルでは現天候表示＆タップで切替) */}
        <div className="hidden sm:flex glass-panel px-2.5 py-1.5 rounded-2xl items-center gap-2 bg-slate-950/80 backdrop-blur-md border border-white/15">
        ...
        {/* 🎵 Gemini AI BGMプレイヤー */}
        <div className="glass-panel px-2 sm:px-2.5 py-1.5 rounded-2xl flex items-center gap-1.5 sm:gap-2 bg-slate-950/80 backdrop-blur-md border border-white/15">
          <button
            onClick={onToggleBgm}
            onTouchEnd={(e) => {
              e.preventDefault();
            ...
            className={`flex items-center gap-1 sm:gap-1.5 transition-all cursor-pointer active:scale-95 ${
          ...
          >
            ...
            <span className="max-w-[120px] truncate text-[11px] font-medium hidden md:inline">
        ...
        {/* 設定・ツールボタン群 */}
        <div className="glass-panel p-1 sm:p-1.5 rounded-2xl flex items-center gap-1 bg-slate-950/80 backdrop-blur-md border border-white/15">
          {/* ⚡ 低負荷モード切り替え (低スペックPC用) */}
          <button
          ...
          <button
            ...
            className="hidden sm:inline-flex p-1.5 rounded-xl hover:bg-white/10 text-slate-300 hover:text-white transition-all text-xs font-mono active:scale-95 cursor-pointer"
            title="カメラリセット"
          >
            100%
          </button>
          ...
          <button
            ...
            className="hidden sm:inline-flex p-1.5 rounded-xl hover:bg-white/10 text-slate-300 hover:text-white transition-all active:scale-95 cursor-pointer"
            title="操作方法の確認"
          >
